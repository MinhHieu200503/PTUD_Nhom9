...
//            }
            ...
            public void onSubtract(int row) {
               ...
               else  model.setValueAt( sl, row, 2);
            }
...
//       
    ...
    private void themDichVu(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_themDichVu
        if (evt.getClickCount() == 2) { // Kiểm tra xem là double click
            ...
            int vitri = isTonTai(dv.getTenDichVu());
            
            if(vitri != -1){
                int slMoi = Integer.parseInt(model_dv_dangsudung.getValueAt(vitri, 2).toString()) + Integer.parseInt(sl);
                model_dv_dangsudung.setValueAt(slMoi, vitri, 2);
            }
            else{
    ...
    public int isTonTai (String tenDV){
        
        DefaultTableModel modelDVdgsd = (DefaultTableModel) table_dvPhongDangSuDung.getModel();
        for ( int i=0; i<modelDVdgsd.getRowCount(); i++){
            if(tenDV.equals(modelDVdgsd.getValueAt(i, 0)))
                return i;
        }
        return -1;
    }
